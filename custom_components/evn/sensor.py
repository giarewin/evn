...
# Nhãn tiếng Việt cho tên hiển thị
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, add_entities: AddEntitiesCallback):
    ...
    for k in SENSORS:
        # Entity ID CHUẨN theo yêu cầu: sensor.<prefix>_<buy|sell>_<day|month|year>
        entity_slug = f"{slug}_{k.kind}_{k.period}"
        # Friendly name hiển thị: "Prefix Mua (Ngày)" ...
        friendly = f"{prefix} {VN_KIND[k.kind]} ({VN_PERIOD[k.period]})"
...
class EvnDerivedSensor(SensorEntity):
    _attr_has_entity_name = False                 # dùng nguyên tên thân thiện đã đặt
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry, runtime, entity_slug: str, friendly: str, kind: str, period: str) -> None:
        ...
        # ÉP entity_id theo format yêu cầu (trước khi entity được add vào HA)
