...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, add_entities: AddEntitiesCallback):
    ...
    for k in SENSORS:
        # entity_id/unique_id ổn định dựa trên slug
        entity_slug = f"{slug}_{k.kind}_{k.period}"
        # Friendly name theo yêu cầu: "Prefix Mua (Ngày)",...
...
class EvnDerivedSensor(SensorEntity):
    _attr_has_entity_name = False                 # dùng nguyên tên thân thiện đã chuẩn hóa
    _attr_entity_category = EntityCategory.DIAGNOSTIC  # gợi ý: có thể exclude khỏi recorder
    _attr_should_poll = False                     # cập nhật qua callback
