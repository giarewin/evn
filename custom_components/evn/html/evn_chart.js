...
// Nếu plugin zoom expose global (khi load qua <script>), đăng ký thêm
...
let currentMode  = "year"; // mặc định: Tháng
...
function formatKwhValue(v) {
  if (!v || v <= 0) return "";
  if (currentMode === "day")   return v.toFixed(2);     // Giờ: 2 số lẻ
  if (currentMode === "month") return v.toFixed(1);     // Ngày: 1 số lẻ
  return String(Math.round(v));                         // Tháng/Năm: nguyên
}

function formatMoneyValue(v) {
  if (!v || v <= 0) return "";
  if (currentMode === "day")   return v.toFixed(2) + "K";  // Giờ: 2 số lẻ
  if (currentMode === "month") return v.toFixed(1) + "K";  // Ngày: 1 số lẻ
  return String(Math.round(v)) + "K";                      // Tháng/Năm: nguyên
}
...
async function readCsvFilteredByDate(dateFilter) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const buyCostVal  = idxBuyHourCost >= 0 ? parseFloat(cols[idxBuyHourCost]) : 0;
    const sellCostVal = idxSellHourCost >= 0 ? parseFloat(cols[idxSellHourCost]) : 0;
...
async function readCsvByMonth(monthFilter) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const buyCostVal  = idxBuyDayCost  >= 0 ? parseFloat(cols[idxBuyDayCost])  : 0;
    const sellCostVal = idxSellDayCost >= 0 ? parseFloat(cols[idxSellDayCost]) : 0;
...
async function readCsvByYear(yearFilter) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const buyCostVal  = idxBuyMonthCost  >= 0 ? parseFloat(cols[idxBuyMonthCost])  : 0;
    const sellCostVal = idxSellMonthCost >= 0 ? parseFloat(cols[idxSellMonthCost]) : 0;
...
async function readCsvYearSum() {
  ...
  for (let i = 0; i < AVAILABLE_YEARS.length; i++) {
    ...
    try {
      ...
      const buyCostVal  = idxBuyYearCost  >= 0 ? parseFloat(cols[idxBuyYearCost])  : 0;
      const sellCostVal = idxSellYearCost >= 0 ? parseFloat(cols[idxSellYearCost]) : 0;
...
async function initChart() {
  ...
  if (saved.mode && isValidMode(saved.mode)) {
    ...
    currentMode = "year"; // mặc định Tháng
  }
  ...
  const gridColor = getComputedStyle(document.documentElement)
    .getPropertyValue("--grid-color") || "rgba(156,163,175,0.6)";
  const axisColor = getComputedStyle(document.documentElement)
    .getPropertyValue("--axis-text") || "#4b5563";
  const labelColor = getComputedStyle(document.documentElement)
    .getPropertyValue("--label-text") || "#111827";
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          ...
          afterDataLimits(scale) {
            ...
            if (max <= 1) {
              const up = Math.ceil(max * 10) / 10;
              scale.max = up > 0 ? up : 0.1;
            } else {
              scale.max = max + 1;
            }
          },
      ...
      plugins: {
        ...
        datalabels: {
          labels: {
            kwh: {
              ...
              font: { size: 11, weight: "400" },   // bỏ in đậm
              offset: 2
            },
            cost: {
              color: labelColor,
              ...
              font: { size: 10, weight: "400" },  // bỏ in đậm
              offset: 0
            }
          }
        },
        tooltip: {
          ...
          displayColors: true,
          callbacks: {
            // ====== TIÊU ĐỀ TOOLTIP (GIỜ / NGÀY / THÁNG / NĂM) ======
            title(items) {
            ...
            label(context) {
              ...
              // Mua/Bán: kWh
              parts.push(` ${context.dataset.label}: ${formatTooltipKwh(v)} kWh`);

              // Giá: có ô vuông phía trước
              if (cost > 0) {
        ...
        // ====== THÊM MỚI: pinch-zoom ngang bằng 2 ngón tay ======
        zoom: {
          pan: {
            enabled: false
          },
          zoom: {
            wheel: { enabled: false },   // tắt zoom bằng cuộn chuột
            pinch: { enabled: true },    // bật zoom bằng 2 ngón tay
            drag: { enabled: false },
            mode: "x"                    // chỉ zoom theo trục X (chiều ngang)
          }
        }
      }
    }
...
(async () => {
  ...
  btnDay?.addEventListener("click", () => setMode("day").catch(console.error));
  btnMonth?.addEventListener("click", () => setMode("month").catch(console.error));
  btnYear?.addEventListener("click", () => setMode("year").catch(console.error));
  btnYearSum?.addEventListener("click", () => setMode("yearsum").catch(console.error));
